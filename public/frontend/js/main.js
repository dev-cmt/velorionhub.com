(function ($) {
    "use strict"; var selectImages = function () { if ($(".image-select").length > 0) { const selectIMG = $(".image-select"); selectIMG.find("option").each((idx, elem) => { const selectOption = $(elem); const imgURL = selectOption.attr("data-thumbnail"); if (imgURL) { selectOption.attr("data-content", `<img src="${imgURL}" /> ${selectOption.text()}`) } }); selectIMG.selectpicker() } }; var goTop = function () { var $goTop = $("#goTop"); var $borderProgress = $(".border-progress"); $(window).on("scroll", function () { var scrollTop = $(window).scrollTop(); var docHeight = $(document).height() - $(window).height(); var scrollPercent = (scrollTop / docHeight) * 100; var progressAngle = (scrollPercent / 100) * 360; $borderProgress.css("--progress-angle", progressAngle + "deg"); if (scrollTop > 100) { $goTop.addClass("show") } else { $goTop.removeClass("show") } }); $goTop.on("click", function () { $("html, body").animate({ scrollTop: 0 }, 0) }) }; var headerSticky = function () {
        let lastScrollTop = 0; let delta = 5; let navbarHeight = $("header").outerHeight(); let didScroll = !1; $(window).scroll(function () { didScroll = !0 }); setInterval(function () {
            if (didScroll) {
                let st = $(window).scrollTop(); navbarHeight = $("header").outerHeight(); if (st > navbarHeight) { if (st > lastScrollTop + delta) { $("header").css("top", `-${navbarHeight}px`) } else if (st < lastScrollTop - delta) { $("header").css("top", "0"); $("header").addClass("header-type-stc") } } else { $("header").css("top", "unset"); $("header").removeClass("header-type-stc") }
                lastScrollTop = st; didScroll = !1
            }
        }, 250)
    }; var autoPopup = function () {
        if ($(".auto-popup").length > 0) { let showPopup = sessionStorage.getItem("showPopup"); if (!JSON.parse(showPopup)) { setTimeout(function () { $(".auto-popup").modal("show") }, 3000) } }
    ...
    }; var clickActive = function () { $(".btn-active").click(function (event) { event.stopPropagation(); let container = $(this).closest(".active-container"); let activeItem = container.find(".active-item"); $(this).toggleClass("active"); activeItem.toggleClass("active") }); $(document).click(function (event) { if (!$(event.target).closest(".active-container").length) { $(".btn-active, .active-item").removeClass("active") } }) }; var formatCardNumber = function () {
        $(".number-credit-card").on("input", function () {
            var value = $(this).val().replace(/\D/g, ""); if (value.length > 16) { value = value.slice(0, 16) }
            $(this).val(value.replace(/(\d{4})(?=\d)/g, "$1 "))
        })
    }; var changeBgImg = function () { $(".has-bg-img").each(function () { var bgImage = $(this).data("bg-img"); var bgPosition = $(this).data("bg-position") || "left"; if (bgImage) { $(this).css({ "background-image": "url(" + bgImage + ")", "background-position": bgPosition, }) } }) }; var switchLayout = function () {
        $(".tf-view-layout-switch").on("click", function (e) {
            e.preventDefault(); var tabClass = $(this).data("tab"); var gridLayout = $(".tf-grid-layout"); var gridLayoutWrapper = $(".gridLayout-wrapper"); var cardProduct = $(".tf-grid-layout .card-product"); var viewSwitch = $(this); gridLayoutWrapper.addClass("loading"); gridLayout.addClass("d-none"); setTimeout(function () {
                gridLayout.removeClass(function (index, className) { return (className.match(/layout-\S+/g) || []).join(" ") }); gridLayout.addClass("layout-" + tabClass); $(".tf-view-layout-switch").removeClass("active"); viewSwitch.addClass("active"); cardProduct.attr("class", "card-product style-img-border"); if (tabClass === "tablist-1") { cardProduct.addClass("style-row") } else if (tabClass === "tablist-2") { cardProduct.addClass("style-row type-row-2 row-small flex-sm-row") }
                gridLayoutWrapper.removeClass("loading"); gridLayout.removeClass("d-none")
            }, 300)
        }); $(".select-item").on("click", function (e) { e.preventDefault(); var gridLayout = $(".tf-grid-layout"); var gridLayoutWrapper = $(".gridLayout-wrapper"); gridLayoutWrapper.addClass("loading"); gridLayout.addClass("d-none"); setTimeout(function () { gridLayoutWrapper.removeClass("loading"); gridLayout.removeClass("d-none") }, 300) })
    }; const preloader = function () { $("#preload").fadeOut("slow", function () { $(this).remove() }) }; var updatePriceMinicart = function () {
        function updateTotal() { let total = 0; $(".popup-shopping-cart .new-price").each(function () { let price = parseFloat($(this).text().replace(/[$,]/g, "")); if (!isNaN(price)) { total += price } }); $(".popup-shopping-cart .price-amount").text(`$${total.toFixed(3)}`) }
        ...
        updateTotal(); checkCartEmpty(); $(".remove").on("click", function () { $(this).closest(".file-delete").remove(); updateTotal(); checkCartEmpty() })
    }; var compareEmpy = function () {
        function updateCompareUI() { let compareItems = $(".offcanvas-compare .tf-compare-item"); let emptyState = $(".mini-compare-empty"); let compareWrap = $(".tf-compare-wrap"); if (compareItems.length === 0) { emptyState.show(); compareWrap.hide() } else { emptyState.hide(); compareWrap.show() } }
        ...
        function updateWishlistUI() { let wishlistItems = $(".tf-table-wishlist tbody tr"); let wishlistFooter = $(".tf-table-wishlist tfoot"); if (wishlistItems.length === 0) { wishlistFooter.removeClass("d-none") } else { wishlistFooter.addClass("d-none") } }
        updateWishlistUI(); $(".wishlist-item_remove .remove").on("click", function () { $(this).closest(".wishlist-item").remove(); updateWishlistUI() })
