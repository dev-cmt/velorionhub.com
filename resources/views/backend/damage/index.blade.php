                    ...
                    <tbody>
                        @forelse($damages as $key => $item)
                            <tr>
                                ...
                                <td>{{ $item->get_product->name ?? 'N/A' }} <span
                                        class="text-info">#{{ $item->sku }}</span></td>
                                <td>{{ $item->quantity }}</td>
                                <td>{{ number_format($item->amount, 2) }}</td>
                                <td>{{ $item->note }}</td>
                                <td class="text-center">
                                    <a href="javascript:void(0);" onclick="editForm({{ $item->id }})"
                                        class="text-primary me-2"><i class="ri-edit-line"></i></a>
                                    <a href="javascript:void(0);" onclick="deleteForm({{ $item->id }})"
                                        class="text-danger"><i class="ri-delete-bin-5-line"></i></a>
                                </td>
                            </tr>
                        @empty
                            <tr>
                            ...
                            <select class="form-select select2" name="product_id" id="product_id" required>
                                ...
                                @foreach ($products as $product)
                                    <option value="{{ $product['id'] }}" data-sku="{{ $product['sku'] }}">
                                        {{ $product['name'] }} ({{ $product['total_stock'] }}pc)</option>
                                @endforeach
                            </select>
