<x-backend-layout title="Stores Management">
    ...
    <div class="card custom-card">
        <div class="card-header justify-content-between py-2" style="background-color: #e6533c3d !important;color: red">
            ...
            <a href="{{ route('return.receive.clear.temp') }}" onclick="return confirm('Are you sure to clear?')"
                    ...
                    <div class="row">
                        <form action="" method="get" class="d-flex gap-2 align-items-center">
                            <div class="flex-grow-1">
                                <input type="text" name="query" class="form-control form-control-sm" required
                                    id="query" value="{{ request()->query('query') ?? '' }}"
                        ...
                        @if (count($parcels) > 0)
                            @foreach ($parcels as $key => $parcel)
                                <tr>
                                    ...
                                    <td>{{ $parcel->get_sale->get_courier ? $parcel->get_sale->get_courier->name : '---' }}
                                    </td>
