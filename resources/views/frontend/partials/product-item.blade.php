<div class="product-card">
    <form method="post" action="{{ route('cart.add') }}" class="ms2_form">
        ...
        <div class="product-card__actions-list">
            <button class="product-card__action product-card__action--wishlist msWishlistTogglePage"
                type="button" aria-label="Add to wish list" onclick="addToWishlist({{ $product->id }})">
            ...
            <button class="product-card__action product-card__action--compare msCompareTogglePage" 
                type="button" aria-label="Add to compare" onclick="addToCompare({{ $product->id }})">
                <svg width="16" height="16">
    ...
    <form method="post" action="{{ route('cart.add') }}" class="ms2_form">
        ...
        <div class="product-card__footer">
            <div class="product-card__prices">
                @if($product->sale_price < $product->regular_price)
                    <div class="product-card__price product-card__price--new">TK {{ number_format($product->sale_price, 2) }}</div>
                    <div class="product-card__price product-card__price--old">TK {{ number_format($product->regular_price, 2) }}</div>
                @else
                    <div class="product-card__price product-card__price--current">TK {{ number_format($product->sale_price, 2) }}</div>
                @endif
            </div>
